...
private const val MCP_TOOL_MAX_ATTEMPTS = 2
...
class McpManager(
    ...
    private val clients: MutableMap<McpServerConfig, Client> = ConcurrentHashMap()
    ...
    suspend fun callTool(toolName: String, args: JsonObject): List<UIMessagePart> {
        ...
        val preparedArgs = tool.inputSchema.prepareArguments(args)
        if (preparedArgs.errors.isNotEmpty()) {
            return listOf(
                UIMessagePart.Text(
                    JsonInstant.encodeToString(
                        buildJsonObject {
                            put("error", "Invalid tool arguments: ${preparedArgs.errors.joinToString(separator = "; ")}")
                        }
                    )
                )
            )
        }
        val normalizedArgs = preparedArgs.arguments as? JsonObject ?: JsonObject(emptyMap())
        if (preparedArgs.corrections.isNotEmpty()) {
            Log.w(TAG, "callTool: normalized $toolName args: ${preparedArgs.corrections.joinToString()}")
        }
        Log.i(TAG, "callTool: $toolName / $normalizedArgs")

        if (client.transport == null) client.connect(getTransport(config))
        var lastError: Throwable? = null
        repeat(MCP_TOOL_MAX_ATTEMPTS) { attempt ->
            try {
                ...
                lastError = e
                if (!shouldRetryToolCall(e) || attempt == MCP_TOOL_MAX_ATTEMPTS - 1) {
                    throw e
                }
                Log.w(TAG, "callTool: retrying $toolName after ${e.javaClass.simpleName}: ${e.message}")
                runCatching { client.connect(getTransport(config)) }
            }
        }
        throw lastError ?: error("MCP tool call failed")
    }
    ...
    private fun shouldRetryToolCall(error: Throwable): Boolean {
