...
class SettingsStore(
    ...
    val settingsFlowRaw = dataStore.data
        .catch { exception ->
            ...
            Settings(
                ...
                rootEnabled = preferences[ROOT_ENABLED] == true,
                keepAliveEnabled = preferences[KEEP_ALIVE_ENABLED] == true,
                sshEnabled = preferences[SSH_ENABLED] == true,
                sshHost = preferences[SSH_HOST] ?: "",
                sshPort = preferences[SSH_PORT] ?: 22,
                sshUsername = preferences[SSH_USERNAME] ?: "",
                sshAuthType = preferences[SSH_AUTH_TYPE]?.let { JsonInstant.decodeFromString(it) }
    ...
    suspend fun update(settings: Settings) {
        ...
        dataStore.edit { preferences ->
            ...
            preferences[ROOT_ENABLED] = settings.rootEnabled
            preferences[KEEP_ALIVE_ENABLED] = settings.keepAliveEnabled
            preferences[SSH_ENABLED] = settings.sshEnabled
            preferences[SSH_HOST] = settings.sshHost
            preferences[SSH_PORT] = settings.sshPort
            preferences[SSH_USERNAME] = settings.sshUsername
            preferences[SSH_AUTH_TYPE] = JsonInstant.encodeToString(settings.sshAuthType)
