...
const val ROOT_TERMINAL_NOTIFICATION_CHANNEL_ID = "root_terminal"
...
class RikkaHubApp : Application() {
    override fun onCreate() {
        ...
        get<AppScope>().launch(Dispatchers.IO) {
            runCatching {
                get<RootManager>().refreshStatus()
            }.onFailure {
                Log.e(TAG, "root preflight failed", it)
            }
        }
        ...
        // Start keep-alive service (always on)
    ...
    private fun startKeepAliveService() {
        get<AppScope>().launch {
            runCatching {
                delay(1000)
                val intent = Intent(this@RikkaHubApp, me.rerere.rikkahub.service.KeepAliveService::class.java).apply {
                    action = me.rerere.rikkahub.service.KeepAliveService.ACTION_START
                }
                startForegroundService(intent)
            }.onFailure {
                Log.e(TAG, "startKeepAliveService failed", it)
            }
        }
    }
    ...
    private fun createNotificationChannel() {
        ...
        val rootTerminalChannel = NotificationChannelCompat
            .Builder(ROOT_TERMINAL_NOTIFICATION_CHANNEL_ID, NotificationManagerCompat.IMPORTANCE_LOW)
            .setName(getString(R.string.notification_channel_root_terminal))
            .setVibrationEnabled(false)
            .setShowBadge(false)
            .build()
        notificationManager.createNotificationChannel(rootTerminalChannel)
