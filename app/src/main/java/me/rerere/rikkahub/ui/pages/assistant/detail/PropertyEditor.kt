...
fun CustomBodies(customBodies: List<CustomBody>, onUpdate: (List<CustomBody>) -> Unit) {
    ...
    Column(
        ...
        customBodies.forEachIndexed { index, body ->
            ...
            var highlightTokens by remember { mutableStateOf<List<HighlightToken>?>(null) }

            LaunchedEffect(bodyValueString) {
                scope.launch {
                    runCatching {
                        highlightTokens = withContext(Dispatchers.IO) {
                            highlighter.highlight(bodyValueString, "json")
                        }
                    }
                }
            }
